...
public class Section{
	...
	public Section(){
		for(int i = 0; i < AplConfig.NUMBER_OF_ROWS_PER_SECTION; i++){
			addRow(AplConfig.NUMBER_OF_SEATS_PER_ROW);
		}
	}
	
	/**
	 * Creates a Section with a number of rows and a number of seats for a row.
	 * @param nrOfRows the number of rows the section will have
	 * @param nrOfSeatsPerRow the number of seats a row will have
	 */
	 ...
	 * @param nrOfSeats the amount of seats the added row is suppossed to have
	 */
	 ...
	 * @param nrOfPersons the number of people a reservation needs to be made for
	 * @param userId the is of the person that is reserving the seats
	 ...
	 * @param reservation the reservation that needs to be cancelled
	 */
	 ...
	 * @param reservation the reservation that needs to be confirmed a payed
	 */
	...
	/**
	 * Generates a String representation of the sections bought seats
	 * the first line represents the seat numbers
	...
	public String stringSection(){
		...
		if(rows.size() != 0){
			b.append("S<");
			int idSize = (AplConfig.NUMBER_OF_CUSTOMERS+"").length();
